...
from model_class import BasicModel
...
#     test_csv = 'val.csv'
    ...
    def process(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        with open('lib/vocab.pickled', 'rb') as f:
            inp_voc = pickle.load(f)
        
        model = BasicModel(inp_voc)
        if device != 'cuda':
            model.load_state_dict(torch.load('lib/model.torch', map_location=torch.device('cpu')))
        else:
            model.load_state_dict(torch.load('lib/model.torch'))
        model.eval()
        batch_size = 8
        with torch.no_grad():
            y_pred = np.array([])
            for i in range(0, len(test_text), batch_size):
                y_pred = np.append(y_pred, model(inp_voc.to_matrix(test_text[i: i + batch_size]).to(device)).squeeze().cpu().numpy())
        
        
        task1_prediction = pd.DataFrame(columns=['index', 'prediction'])
        task1_prediction['index'] = test.index
        task1_prediction['prediction'] = y_pred
